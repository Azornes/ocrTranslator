...
class TabviewChats(customtkinter.CTkTabview):
    def __init__(self, root, ai_services, send_message_icon, display_func = None):
        ...
        for ai_service in ai_services:
            ...
            loading_icon_chat = AnimatedGif(textbox_ai_send_frame, assets.path_to_loading_gif, 0.04, size=(50, 50))
            loading_icon_chat.grid(row=0, column=1, padx=(14, 7), pady=(4, 0), sticky="e")
            loading_icon_chat.grid_save()
            ...
            method_name = 'loading_icon_chat_{}'.format(ai_service[0].lower().replace(" ", "_"))
            setattr(self, method_name, loading_icon_chat)
    ...
    def send_message_ai(self, textbox_ai_send_frame, textbox_ai_frame, name_service):
        method_name = 'self.loading_icon_chat_{}'.format(name_service.lower())
        method2 = eval(method_name)
        method2.start()
        method_name = 'self.button_send_message_{}'.format(name_service.lower())
        method3 = eval(method_name)
        method3.configure(state="disabled")
        ...
        method2.stop()
        method3.configure(state="normal")
