...
Edited by Azornes to working with customtkinter library (https://github.com/TomSchimansky/CustomTkinter)
"""
...
class AutocompleteCombobox(tk.CTkComboBox):
    ...
    def __init__(self, master=None, completevalues=None, **kwargs):
        ...
        tk.CTkComboBox.__init__(self, master, values=completevalues, **kwargs)
...
""" % (self._entry))
    ...
    def autocomplete(self, delta=0):
        ...
        if delta:  # need to delete selection otherwise we would fix the current position
            self._entry.delete(self.position, tk.END)
        ...
        if self._hits:
            ...
            self._entry.select_range(self.position, tk.END)
    ...
    def handle_keyrelease(self, event):
        ...
        if event.keysym == "BackSpace":
            self._entry.delete(self._entry.index(tk.INSERT), tk.END)
            self.position = self._entry.index(tk.END)
        if event.keysym == "Left":
            if self.position < self._entry.index(tk.END):  # delete the selection
                self._entry.delete(self.position, tk.END)
            else:
                self.position -= 1  # delete one character
                self._entry.delete(self.position, tk.END)
        if event.keysym == "Right":
            self.position = self._entry.index(tk.END)  # go to end (no selection)
    ...
    def cget(self, key):
        ...
        return tk.CTkComboBox.cget(self, key)

    def keys(self):
        """Return a list of all resource names of this widget."""
        keys = tk.CTkComboBox.keys(self)
